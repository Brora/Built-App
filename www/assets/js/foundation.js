...
! function(window, document, undefined) {
    ...
    function formatNumber(number, pattern, groupSep, decimalSep, fractionSize) {
        ...
        if (hasExponent) fractionSize > 0 && 1 > number && (formatedText = number.toFixed(fractionSize), number = parseFloat(formatedText));
        else {
            ...
            ! function(b, e) {
                ...
                var g = "file:" === b.location.protocol,
                    ...
                    s = function(a, c, d, e) {
                        ...
                        if (!/svg$/i.test(g)) return void e("Attempted to inject a file with a non-svg extension: " + g);
                        if (!h) {
                            var j = a.getAttribute("data-fallback") || a.getAttribute("data-png");
                            return void(j ? (a.setAttribute("src", j), e(null)) : d ? (a.setAttribute("src", d + "/" + g.split("/").pop().replace(".svg", ".png")), e(null)) : e("This browser does not support SVG and no PNG fallback was defined."))
...
! function() {
    ...
    function FoundationMQInit(helpers, foundationApi, u) {
        function init() {
            ...
            helpers.headerHelper(["foundation-mq"]), extractedMedia = helpers.getStyle(".foundation-mq", "font-family"), mediaQueries = helpers.parseStyleToObject(extractedMedia);
        ...
        var factory = {};
        return factory.init = init, factory
    }
    ...
    function mqHelpers() {
        ...
        var factory = {};
        return factory.headerHelper = headerHelper, factory.getStyle = getStyle, factory.parseStyleToObject = parseStyleToObject, factory
    }
    ...
    function FoundationApi() {
        ...
        function closeActiveElements(options) {
            ...
            activeElements.length && angular.forEach(activeElements, function(el) {
                options.exclude !== el.id && self.publish(el.id, "close")
            })
        }

        function animate(element, futureState, animationIn, animationOut) {
            function cancelAnimation() {
                deregisterElement(element), element.off(events.join(" ")), timedOut = !1
            }
            ...
            function registerElement(el) {
                var elObj = {
                    ...
                    inProgress = animations.filter(function(obj) {
                        return obj.el === el
                    });
                inProgress.length > 0 && inProgress[0].animation.cancelAnimation(), animations.push(elObj)
            }

            function deregisterElement(el) {
                {
                    var index;
                    animations.filter(function(obj, ind) {
                        obj.el === el && (index = ind)
                    })
                }
                index >= 0 && animations.splice(index, 1)
            }
            ...
            function animateElement(animationClass, activation) {
                var initClass = activation ? initClasses[0] : initClasses[1],
                    ...
                    finishAnimation = function() {
                        deregisterElement(element), reset(), element.removeClass(activation ? "" : activeGenericClass), reflow(), timedOut = !1
                    };
                registerElement(element), reset(), element.addClass(animationClass), element.addClass(initClass), element.addClass(activeGenericClass), reflow(), element[0].style.transitionDuration = "", element.addClass(activeClass), element.one(events.join(" "), function() {
                    finishAnimation()
                }), setTimeout(function() {
                    timedOut && finishAnimation()
                }, 3e3)
            }
            var initClasses = ["ng-enter", "ng-leave"],
                ...
                timedOut = !0,
                self = this;
            self.cancelAnimation = cancelAnimation, animateElement(futureState ? animationIn : animationOut, futureState)
        }
        var listeners = [],
            settings = {},
            uniqueIds = [],
            animations = [],
            service = {};
        return service.subscribe = subscribe, service.publish = publish, service.getSettings = getSettings, service.modifySettings = modifySettings, service.generateUuid = generateUuid, service.toggleAnimation = toggleAnimation, service.closeActiveElements = closeActiveElements, service.animate = animate, service
    }
    ...
    function Utils() {
        ...
        var utils = {};
        return utils.prepareRoute = prepareRouteUtil, utils.throttle = throttleUtil, utils
    }
    angular.module("foundation.core", []).run(mqInitRun).factory("FoundationMQInit", FoundationMQInit).factory("mqHelpers", mqHelpers).service("FoundationApi", FoundationApi).filter("prepareRoute", prepareRoute).factory("Utils", Utils), mqInitRun.$inject = ["FoundationMQInit"], FoundationMQInit.$inject = ["mqHelpers", "FoundationApi", "Utils"]
}();
! function() {
    "use strict";

    function uiAnimation($rootScope) {
        function enterAnimation(element, done) {
            ...
            if (scope.vars && scope.vars.animationIn) {
                var animationIn = scope.vars.animationIn,
                    ...
                    timedOut = !0;
                element.parent().addClass(parentStyle), element.removeClass(activate + " " + initial + " " + animationIn + " " + animationOut), element[0].style.transitionDuration = 0, element.addClass(animationIn), element.addClass(initial), $rootScope.$digest(), element[0].style.transitionDuration = "", element.addClass(activate);
                var finishAnimation = function() {
                    element.parent().removeClass(parentStyle), element.removeClass(activate + " " + initial + " " + animationIn + " " + animationOut), timedOut = !1, done()
                };
        ...
        function leaveAnimation(element, done) {
            ...
            if (scope.vars && scope.vars.animationOut) {
                var animationIn = scope.vars.animationIn || "",
                    ...
                    timedOut = !0;
                element.removeClass(activate + " " + initial + " " + animationIn + " " + animationOut), element[0].style.transitionDuration = 0, element.addClass(animationOut), element.addClass(initial), $rootScope.$digest(), element[0].style.transitionDuration = "", element.addClass(activate);
                var finishAnimation = function() {
                    element.parent().removeClass(parentStyle), element.removeClass(activate + " " + initial + " " + animationIn + " " + animationOut), timedOut = !1, done()
                };
                element.one(events.join(" "), function() {
        ...
        var events = ["webkitAnimationEnd", "mozAnimationEnd", "MSAnimationEnd", "oanimationend", "animationend", "webkitTransitionEnd", "otransitionend", "transitionend"],
            ...
            animation = {};
        return animation.enter = enterAnimation, animation.leave = leaveAnimation, animation
    }
    angular.module("foundation.dynamicRouting.animations", ["ngAnimate", "foundation.dynamicRouting"]).animation(".ui-animation", uiAnimation), uiAnimation.$inject = ["$rootScope", "$state"]
}();
! function() {
    "use strict";
    ...
    function FoundationState($stateProvider) {
        ...
        function getController(state) {
            ...
            return /\w\s+as\s+\w/.test(ctrl) || (ctrl += " as PageCtrl"), ctrl
        }
        var complexViews = {};
        this.registerDynamicRoutes = function(routes) {
            ...
            angular.forEach(dynamicRoutes, function(page) {
                if (page.hasComposed) angular.isDefined(complexViews[page.parent]) || (complexViews[page.parent] = {
                    children: {}
                }), page.controller && (page.controller = getController(page)), complexViews[page.parent].children[page.name] = page;
                else if (page.composed) angular.isDefined(complexViews[page.name]) || (complexViews[page.name] = {
                    children: {}
                }), page.controller && (page.controller = getController(page)), angular.extend(complexViews[page.name], page);
                else {
                    var state = {
                        url: page.url,
...
! function() {
    ...
    function zfAccordionController($scope) {
        var controller = this,
            ...
            collapsible = (controller.multiOpen = $scope.multiOpen = $scope.multiOpen || !1, controller.collapsible = $scope.collapsible = $scope.multiOpen || $scope.collapsible || !0),
            autoOpen = controller.autoOpen = $scope.autoOpen = $scope.autoOpen || !0;
        controller.select = function(selectSection) {
            sections.forEach(function(section) {
                controller.multiOpen ? section.scope === selectSection && (section.scope.active = !section.scope.active) : section.scope.active = section.scope === selectSection ? collapsible ? !section.scope.active : !0 : !1
            })
        }, controller.addSection = function(sectionScope) {
            sections.push({
                scope: sectionScope
            }), 1 === sections.length && autoOpen === !0 && (sections[0].active = !0, sections[0].scope.active = !0)
    ...
    function zfAccordion() {
        function link(scope, element, attrs, controller) {
            scope.multiOpen = controller.multiOpen = "true" === scope.multiOpen ? !0 : !1, scope.collapsible = controller.collapsible = "true" === scope.collapsible ? !0 : !1, scope.autoOpen = controller.autoOpen = "true" === scope.autoOpen ? !0 : !1
        }
        var directive = {
            restrict: "EA",
            transclude: "true",
    ...
    angular.module("foundation.accordion", []).controller("ZfAccordionController", zfAccordionController).directive("zfAccordion", zfAccordion).directive("zfAccordionItem", zfAccordionItem), zfAccordionController.$inject = ["$scope"]
}();
! function() {
    "use strict";
    ...
    function zfActionSheetController($scope) {
        ...
        controller.registerContent = function(scope) {
            ...
            content.toggle(), container.toggle(), content.$apply(), container.$apply()
    ...
    function zfActionSheet(foundationApi) {
        function compile() {
            ...
            function postLink(scope, element, attrs, controller) {
                ...
                attrs.$set("id", id), scope.active = !1, foundationApi.subscribe(id, function(msg) {
                    "toggle" === msg && controller.toggle(), ("hide" === msg || "close" === msg) && controller.hide()
                }), controller.registerContainer(scope), scope.toggle = function() {
    ...
    angular.module("foundation.actionsheet", ["foundation.core"]).controller("ZfActionSheetController", zfActionSheetController).directive("zfActionSheet", zfActionSheet).directive("zfAsContent", zfAsContent).directive("zfAsButton", zfAsButton), zfActionSheetController.$inject = ["$scope", "FoundationApi"], zfActionSheet.$inject = ["FoundationApi"], zfAsContent.$inject = ["FoundationApi"], zfAsButton.$inject = ["FoundationApi"]
}();
! function() {
    "use strict";
    ...
    function zfEscClose(foundationApi) {
        function link(scope, element) {
            element.on("keyup", function(e) {
                27 === e.keyCode && foundationApi.closeActiveElements(), e.preventDefault()
            })
        }
        var directive = {
            restrict: "A",
    ...
    function zfSwipeClose(foundationApi) {
        function link($scope, element, attrs) {
            ...
            switch (Hammer && (hammerElem = new Hammer(element[0]), hammerElem.get("swipe").set({
    ...
    function zfHardToggle(foundationApi) {
        function link(scope, element, attrs) {
            element.on("click", function(e) {
                ...
                0 !== animatedElements.length ? e.preventDefault() : (foundationApi.closeActiveElements({
                    exclude: attrs.zfHardToggle
                }), foundationApi.publish(attrs.zfHardToggle, "toggle"), e.preventDefault())
            })
        }
        var directive = {
            restrict: "A",
...
! function() {
    ...
    function zfInterchange(foundationApi, $compile, $http, $templateCache) {
        function link(scope, element, attrs, ctrl, transclude) {
            ...
            function matched() {
                ...
                if (count > 0)
                    for (; count--;) {
                        ...
                        mq = matchMedia(namedQueries[rule] ? namedQueries[rule] : rule), mq.matches && matches.push({
                            ind: count
                        })
                    }
                return matches
            ...
            var childScope, current, scenarios, innerTemplates, namedQueries = {
                    "default": "only screen",
            ...
            namedQueries = angular.extend(namedQueries, globalQueries), foundationApi.subscribe("resize", function() {
    ...
    angular.module("foundation.interchange", ["foundation.core"]).directive("zfInterchange", zfInterchange), zfInterchange.$inject = ["FoundationApi", "$compile", "$http", "$templateCache", "$animate"]
}();
! function() {
    "use strict";
    ...
    function iconic() {
        ...
        var iconicObject = IconicJS(),
            service = {};
        return service.getAccess = getAccess, service
    }

    function zfIconic(iconic) {
        function link(scope, element, attrs) {
            var ico = iconic.getAccess();
            scope.dynSrc ? attrs.$set("data-src", scope.dynSrc) : attrs.$set("data-src", attrs.src), ico.inject(element[0])
        }
        var directive = {
            restrict: "A",
            scope: {
    ...
    angular.module("foundation.iconic", []).service("Iconic", iconic).directive("zfIconic", zfIconic), zfIconic.$inject = ["Iconic"]
}();
! function() {
    "use strict";
    ...
    function modalDirective(foundationApi) {
        function compile() {
            ...
            function postLink(scope, element, attrs) {
                function animate() {
                    scope.overlay || element.css("background", "transparent"), foundationApi.animate(element, scope.active, overlayIn, overlayOut), foundationApi.animate(dialog, scope.active, animationIn, animationOut)
                }
                var dialog = angular.element(element.children()[0]);
                scope.active = !1, scope.overlay = "true" === attrs.overlay || "true" === attrs.overlayClose ? !0 : !1, scope.overlayClose = "true" === attrs.overlayClose ? !0 : !1;
    ...
    angular.module("foundation.modal", ["foundation.core"]).directive("zfModal", modalDirective), modalDirective.$inject = ["FoundationApi"]
}();
! function() {
    "use strict";
    ...
    function ZfNotificationController($scope, foundationApi) {
        var controller = this;
        controller.notifications = $scope.notifications = [], controller.addNotification = function(info) {
            var id = foundationApi.generateUuid();
            info.id = id, $scope.notifications.push(info)
            ...
            for (; $scope.notifications.length > 0;) $scope.notifications.pop()
        }
    }
    ...
    angular.module("foundation.notification", ["foundation.core"]).controller("ZfNotificationController", ZfNotificationController).directive("zfNotificationSet", zfNotificationSet).directive("zfNotification", zfNotification).directive("zfNotificationStatic", zfNotificationStatic).directive("zfNotify", zfNotify), ZfNotificationController.$inject = ["$scope", "FoundationApi"], zfNotificationSet.$inject = ["FoundationApi"], zfNotification.$inject = ["FoundationApi"], zfNotificationStatic.$inject = ["FoundationApi"], zfNotify.$inject = ["FoundationApi"]
}();
! function() {
    "use strict";
    ...
    angular.module("foundation.offcanvas", ["foundation.core"]).directive("zfOffcanvas", zfOffcanvas), zfOffcanvas.$inject = ["FoundationApi"]
}();
! function() {
    "use strict";
    ...
    function zfPanel(foundationApi) {
        function compile() {
            function preLink(scope, iElement, iAttrs) {
                iAttrs.$set("zf-closable", type), scope.position = scope.position || "left", scope.positionClass = "panel-" + scope.position
            }

            function postLink(scope, element, attrs) {
                scope.active = !1;
                var animationIn, animationOut, globalQueries = foundationApi.getSettings().mediaQueries;
                "left" === scope.position ? (animationIn = attrs.animationIn || "slideInRight", animationOut = attrs.animationOut || "slideOutLeft") : "right" === scope.position ? (animationIn = attrs.animationIn || "slideInLeft", animationOut = attrs.animationOut || "slideOutRight") : "top" === scope.position ? (animationIn = attrs.animationIn || "slideInDown", animationOut = attrs.animationOut || "slideOutUp") : "bottom" === scope.position && (animationIn = attrs.animationIn || "slideInUp", animationOut = attrs.animationOut || "slideOutBottom"), foundationApi.subscribe(attrs.id, function(msg) {
                    ...
                    !matchMedia(globalQueries.medium).matches && srcEl.href && srcEl.href.length > 0 && (scope.hide(), foundationApi.animate(element, scope.active, animationIn, animationOut))
                })
            }
            var type = "panel";
            return {
    ...
    angular.module("foundation.panel", ["foundation.core"]).directive("zfPanel", zfPanel), zfPanel.$inject = ["FoundationApi"]
}();
! function() {
    "use strict";
    ...
    function zfPopup(foundationApi) {
        function compile() {
            ...
            function postLink(scope, element, attrs) {
                ...
                scope.active = !1, scope.target = scope.target || !1;
    ...
    angular.module("foundation.popup", ["foundation.core"]).directive("zfPopup", zfPopup).directive("zfPopupToggle", zfPopupToggle), zfPopup.$inject = ["FoundationApi"], zfPopupToggle.$inject = ["FoundationApi"]
}();
! function() {
    "use strict";
...
! function() {
    "use strict";
    angular.module("foundation", ["foundation.core", "foundation.accordion", "foundation.actionsheet", "foundation.common", "foundation.iconic", "foundation.interchange", "foundation.modal", "foundation.notification", "foundation.offcanvas", "foundation.panel", "foundation.popup", "foundation.tabs"])
}();